...
public class PrimMST {
	...
	private void prim(EdgeWeightedGraph G, int s) {
		...
		while (!pq.isEmpty()) {
			int v = pq.delMin();
			if(edgeTo[v] != null){
			int u = edgeTo[v].other(v);
			StdDraw.setPenRadius(.007);
			StdDraw.setPenColor(Color.BLACK);
			StdDraw.line(stars[v].x, stars[v].y, stars[u].x, stars[u].y);// mine
			StdDraw.setPenRadius(.027);
			StdDraw.setPenColor(Color.BLUE);
			StdDraw.point(stars[u].x, stars[u].y);
			StdDraw.setPenRadius(.002);
			StdDraw.setPenColor(Color.BLACK);
			StdDraw.circle(stars[u].x, stars[u].y, .015);
			StdDraw.text(stars[u].x, stars[u].y, "" + u);		
			}
			scan(G, v);
			StdDraw.show(200);
		}
	...
	private void scan(EdgeWeightedGraph G, int v) {
		StdDraw.setPenRadius(.027);
		StdDraw.setPenColor(Color.BLUE);
		StdDraw.point(stars[v].x, stars[v].y);
		StdDraw.setPenRadius(.002);
		StdDraw.setPenColor(Color.BLACK);
		StdDraw.circle(stars[v].x, stars[v].y, .015);
		StdDraw.text(stars[v].x, stars[v].y, "" + v);
		marked[v] = true;
		for (Edge e : G.adj(v)) {
			int w = e.other(v);
			if (marked[w])
			...
			if (e.weight() < distTo[w]) {
				distTo[w] = e.weight();
				edgeTo[w] = e;
				StdDraw.show(50);// wait for better animation
				if (pq.contains(w)){

...
public class QuickUnionUF {
	...
	// Modify for path compression
	// return root of component corresponding to element p
	public int find(int p) {
        int i = p;
        while (p != id[p])
            p = id[p];
        int root = p; //save the root so it can be assigned to the other connections
        int pass; //this will be used to compress the paths of the connections we pass through
        while (i != id[i]) {
                pass = i;
                i = id[i];
                id[pass] = root;
